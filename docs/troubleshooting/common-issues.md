...
# Check events
kubectl get events -n dclaw-monitor --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-monitor deployment/dclaw-monitor-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-monitor-db-credentials -n dclaw-monitor
   ```
