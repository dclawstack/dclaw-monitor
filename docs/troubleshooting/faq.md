...
### How do I update DClaw Monitor?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp monitor --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Monitor without Kubernetes?

Yes, for local development:

```bash
cd dclaw-monitor/frontend && npm run dev
cd dclaw-monitor/backend && uvicorn dclaw_monitor.main:app --reload
```
